...
/* harmony import */ var src_http__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(9827);
var __webpack_async_dependencies__ = __webpack_handle_async_dependencies__([cheerio__WEBPACK_IMPORTED_MODULE_0__, remark__WEBPACK_IMPORTED_MODULE_1__, remark_html__WEBPACK_IMPORTED_MODULE_2__, src_http__WEBPACK_IMPORTED_MODULE_3__]);
([cheerio__WEBPACK_IMPORTED_MODULE_0__, remark__WEBPACK_IMPORTED_MODULE_1__, remark_html__WEBPACK_IMPORTED_MODULE_2__, src_http__WEBPACK_IMPORTED_MODULE_3__] = __webpack_async_dependencies__.then ? (await __webpack_async_dependencies__)() : __webpack_async_dependencies__);




const convertToHTML = async (markdown)=>{
    const processed = await (0,remark__WEBPACK_IMPORTED_MODULE_1__.remark)().use(remark_html__WEBPACK_IMPORTED_MODULE_2__["default"]).process(markdown);
    const stringified = processed.toString();
    return replaceImagesURL(stringified);
};
const replaceImagesURL = (html)=>{
    const $ = (0,cheerio__WEBPACK_IMPORTED_MODULE_0__.load)(html);
    $("img").each((i, img)=>{
        const node = $(img);
        const attrs = node.attr();
        if (attrs && attrs.src) {
            attrs.src = src_http__WEBPACK_IMPORTED_MODULE_3__/* .API_URL */ .T5 + attrs.src;
        }
    });
    return $.html();
};
